...
function verifyToken(req, res, next) {
	...
	if (!token) return res.redirect("src/User/login");

	jwt.verify(token, process.env.JWT_SECRET, function (err, decoded) {
		if (err)
			return res.redirect("src/User/login");
...
app.get("/cuenta", verifyToken, async (req, res) => {
	...
	if (cuenta) {
		...
		res.render("src/User/estado", { saldo: cuenta.saldo, transferencias });
	} else {
		res.status(404).render("src/User/estado", { error: "Cuenta no encontrada" });
	}
});

app.get("/transferencia", verifyToken, (req, res) => {
	res.render("src/User/transferir");
});
...
app.post("/API/register", async (req, res) => {
	...
	if (!nombre || !correo || !contrasena ) {
		return res.status(400).render("src/User/createUser", {
	...
	try {
		...
		res.status(200).render("src/User/index", { usuario: nombre });
	} catch (err) {
		res.status(500).render("src/User/createUser", {
...
app.post("/API/login", async (req, res) => {
	...
	if (user && pass) {s
		...
		if (usuario) {
			...
			res.status(200).render("src/User/index", { usuario: user });
		} else {
			res.render("src/User/login", {
...
app.get("/API/cuenta/:id", verifyToken, async (req, res) => {
	...
	if (cuenta) {
		res.render("src/User/estado", { saldo: cuenta.saldo });
	} else {
		res.status(404).render("src/User/estado", { error: "Cuenta no encontrada" });
	}
});
...
app.post("/API/transferencia", verifyToken, async (req, res) => {
	...
	if (!cuentaEmisor) {
		return res.status(404).render("src/User/transferir", {
	...
	if (cuentaEmisor.saldo < monto) {
		return res.status(400).render("src/User/transferir", {
	...
	res.status(200).render("src/User/transferir", {
